...
// TODO: VERIFY these indexes against your sheet
const COL = {
  id: 0,
  machine: 1,
  shift_n: 2,
...
export function mapRow(row) {
  ...
  return {
    ...
    material: null,                 // add if you map resin later
    pounds,
    machine_hours,
...
async function upsertProductionFacts(pool, records) {
  const table = new sql.Table("#prod_stage");
  table.create = true;
...
export async function runProdExcelIngest({ pool, dry = false } = {}) {
  ...
  // If your sheet includes a header row, drop it here after checking contents:
  const body = rows; // or rows.slice(1)
