...
function canonLine(machine) {
  if (!machine) return '';
  if (!machine) return '';
  // normalize spacing so "Extruder  1" == "Extruder 1"
  const raw = String(machine).trim().replace(/\s+/g, ' ');
...
// ONE route that serves both prod + maint
r.get('/production/dt-reasons', async (req, res, next) => {
  try {
    const pool = await poolPromise;
    if (!pool) return res.json({ reasons: [] });

    const kind   = String(req.query.kind || 'prod').toLowerCase();
    const dim    = String(req.query.dim  || 'cat').toLowerCase() === 'fm' ? 'fm' : 'cat';
    ...
    // --- maintenance: use the SQL proc you built ---
    if (kind === 'maint') {
    ...
    // --- production: keep your legacy computation (exactly like you had) ---
    const weekdaysOnly = wkOnly;
    const isWeekday = (iso) => {
    ...
    // Limit to lines tracked in mappings
    const allow = new Set([
    ...
    // 2) staging reasons per day (raw text -> canonical buckets)
    ...
    for (const r of rawReasons) {
      ...
      for (const p of list) {
        const cat = canonReason(p);
        bag.set(cat, (bag.get(cat) || 0) + 1);
      }
    }

    // 3) allocate residual prod DT
    ...
    for (const [k, agg] of perDayFacts.entries()) {
      ...
      const bag = perDayReasons.get(k);
      if (!bag || bag.size === 0) { add(bucketsProd, 'UNSTATED', resid); continue; }
