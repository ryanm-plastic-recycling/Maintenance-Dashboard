...
async function loadOverallKpis() {
  ...
  for (const asset of mappings.productionAssets || []) {
    ...
    const weekTasksUrl = `${API_V2}/tasks?assets=${id}&status=2&completedDateLte=${weekStart.unix()}&completedDateLte=${weekEnd.unix()}`;
    ...
    const monthTasksUrl = `${API_V2}/tasks?assets=${id}&status=2&completedDateLte=${monthStart.unix()}&completedDateLte=${monthEnd.unix()}`;
    console.log(
