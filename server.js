...
async function loadOverallKpis() {
  ...
  for (const asset of mappings.productionAssets || []) {
    ...
    const weekTasksUrl = `${API_V2}/tasks?assets=${id}&status=2&completedDateGte=${weekStart.unix()}&completedDateGte=${weekEnd.unix()}`;
    ...
    const monthTasksUrl = `${API_V2}/tasks?assets=${id}&status=2&completedDateGte=${monthStart.unix()}&completedDateGte=${monthEnd.unix()}`;
    console.log(
